...
try {
  ...
  /** Same-origin URL for the SvelteKit role landing (`/` or repo base when embedded under `/…/portal/`). */
  ...
  function App() {
    ...
    React.useEffect(() => {
      if (SlateRoute.isFile()) {
        ...
        const internal = typeof SlateRoute.read === 'function' ? SlateRoute.read() : '';
        const atPortalRoute =
          internal !== '' && internal !== 'index.html';
        if (!atPortalRoute && (!location.hash || location.hash.length <= 1)) {
          if (Book.getDemoRole()) SlateRoute.write('dashboard', true);
          else SlateRoute.write('choose-role', true);
        }
      }
