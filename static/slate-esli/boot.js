...
(function slateBoot() {
  ...
  async function run() {
    showHtml(
      '<p style="padding:24px;font-family:system-ui,sans-serif;color:#444">Loading Slate… <span style="font-size:13px;color:#888">(fetching and compiling; first open can take a few seconds)</span></p>'
    );
    try {
      var chunks = [];
      for (var i = 0; i < modules.length; i++) {
        var name = modules[i];
        var url = new URL(name, window.location.href).href;
