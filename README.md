# Slate · Private Wealth — a guided tour

Slate is a prototype workspace for a **Relationship Manager (RM)** at a private wealth firm. An RM is the person at the bank or advisory shop who handles wealthy families end-to-end: knows their goals, signs them up for the right investments, monitors what they own, prepares for every meeting, and writes the proposals that get put in front of them.

This README is written for someone who has never worked in private wealth. Each section first explains the **concepts** in plain English, then shows where they live in the UI so you can click around and see the ideas in action.
...
`/meet/briefing` — what an RM opens first thing. Designed so a colleague picking up the file (RM is on PTO) can read the relationship in 60 seconds.
...
| --- | --- | --- |
| **Upload** (`/meet/upload`) | Drag-and-drop empty state. Lists supported custodians and security badges. | Custodian fragmentation — why aggregation is needed in the first place. |
| **Parsing** (`/meet/parsing`) | Animated 5-step progress: text extraction, custodian reconciliation, CUSIP matching, cost basis, anomaly detection. | The pipeline isn't just OCR — it's also matching **CUSIPs** (the 9-character security ID) to a security master, and reconciling positions across multiple statements. |
| **Parse review** (`/meet/review`) | Table of every parsed position. Some rows are **flagged** because the parser couldn't auto-resolve them (missing tax lot dates, alt funds without a CUSIP, idle cash that crossed a threshold). | Real-world data is messy; the RM needs to confirm before the dashboards are trustworthy. |
...
### Holding detail (`/meet/household/holding/<symbol>`)
...
1. **Start cold:** run `npm run dev` and open `http://localhost:5173/meet/upload`. Drop a fake "PDF" → watch the parsing screen → land on parse review with flagged rows. Confirm. The dashboard appears.
...
5. **The capital call is real:** go to `/investments/alternatives` → Holdings tab. Pine Ridge II shows 84% called, with the next call (Q3 2026, $1.6M) flagged in the schedule. That's the same $1.8M from step 4.
...
7. **Live trade:** go to `/investments/structured-products`. The phoenix RFQ has 8 returned quotes, JPM leading. Play with the Templates tab to see how each shape's payoff differs.
...
- **SvelteKit SPA with real routes.** Run `npm run dev` for local preview; `npm run build` writes a static SPA to `build/`.
- **Tailwind v4 + Slate tokens.** Tailwind is wired through the Vite plugin; `src/app.css` is only the Tailwind entrypoint, theme token mapping, design tokens, and document defaults.
- **Shared UI primitives.** Reused concepts like buttons, cards, tags, tabs, KPI strips, bars, and tables live in `src/lib/components/ui/`. Fast-changing page layout stays as inline Tailwind in the Svelte markup.
- **Scoped app chrome.** Sidebar, topbar, and account-filter styles live in their Svelte components under `src/lib/components/app/`.
- **ES module fixtures.** Fixture data lives in `src/lib/data/fixtures.js`; helper formatting and book math are in `src/lib/domain/format.js` and `src/lib/domain/book.js`.
- **File routes.** Core routes include `/workspace/overview`, `/workspace/clients`, `/workspace/clients/[clientId]/portfolio`, `/meet/upload`, `/meet/household?tab=holdings`, `/meet/household/holding/[symbol]`, `/meet/insights/[insightId]`, `/investments/alternatives`, and `/investments/structured-products`.
- **Persistence:** `localStorage` keys `slate-theme`, `slate-filter`, `slate-loaded`.

| File | Purpose |
| --- | --- |
| `src/routes/+layout.svelte` | App-level shell, breadcrumbs, route-mode detection, persisted state wiring. |
| `src/routes/**/+page.svelte` | SvelteKit file routes that render the correct feature page. |
| `src/lib/components/app/*.svelte` | AppShell, Sidebar, Topbar, and AccountFilter. |
| `src/lib/features/**/*.svelte` | Dashboard, clients, household, briefing, insights, alternatives, structured products, and intake pages. |
| `src/lib/data/fixtures.js` | All fixture data. Mutating values here reshapes the demo. |
| `src/lib/domain/book.js` | Book/client helpers, health scores, AUM splits, and workspace routing. |
| `src/lib/domain/format.js` | Money, percent, dates, and numeric formatting. |
| `src/lib/state/app.js` | Shared Svelte stores for theme, loaded state, account filters, and client roster. |
| `src/app.css` | Tailwind entrypoint, token mapping, design tokens, and base document styles. |
| `svelte.config.js` | Static adapter config with an SPA fallback. |
