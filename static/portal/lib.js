...
/**
 * Paths relative to this SPA's index.html (`…/portal/index.html`) so brand
...
/**
 * Path-based URLs under the portal mount (e.g. …/portal/dashboard, …/portal/clients/…).
 * Legacy #hash bookmarks are migrated once to the path. For file:// opens, routing
 * stays on the hash so the demo works without a dev server.
 */
window.SlateRoute = (function () {
  function isFile() {
    return typeof location !== 'undefined' && location.protocol === 'file:';
  }
  /** Pathname prefix of this SPA: `/org/portal` (no trailing slash). */
  function mountPrefix() {
    if (typeof location === 'undefined') return null;
    var p = decodeURI(location.pathname || '/');
    var m = p.match(/^(.*?\/portal)(?=\/|$)/i);
    return m ? m[1] : null;
  }
  /** Internal route + query (e.g. `dashboard`, `clients/marchetti/pulse?drawer=x`). */
  function read() {
    if (isFile()) {
      var hf = location.hash || '';
      return hf.length > 1 ? hf.slice(1).replace(/^\/+/, '') : '';
    }
    var mount = mountPrefix();
    if (location.hash && location.hash.length > 1) {
      var leg = location.hash.slice(1).replace(/^\/+/, '');
      var hq = leg.indexOf('?');
      var pOnly = hq >= 0 ? leg.slice(0, hq) : leg;
      var hSearch = hq >= 0 ? leg.slice(hq) : '';
      var target = mount ? mount + '/' + pOnly + hSearch : '/' + pOnly + hSearch;
      history.replaceState(null, '', target);
    }
    if (!mount) {
      var full = (location.pathname || '/').replace(/^\/+/, '');
      if (full === '' || full === 'index.html') return (location.search || '').replace(/^\?/, '');
      return full.replace(/^\/+/, '') + (location.search || '');
    }
    var pn = decodeURI(location.pathname || '/');
    var tail = pn.length >= mount.length ? pn.slice(mount.length).replace(/^\/+/, '') : '';
    if (tail === 'index.html' || tail === '') {
      return '';
    }
    var qs = location.search || '';
    if (qs && tail.indexOf('?') < 0) return tail + qs;
    return tail;
  }
  function write(route, replace) {
    ...
    var href;
    if (mount) {
      href = mount + '/' + pathOnly + search;
    } else {
      href = '/' + pathOnly + search;
    }
    if (replace) history.replaceState(null, '', href);
    else history.pushState(null, '', href);
    window.dispatchEvent(new Event('slate-route'));
  }
  return { read, write, isFile, mountPrefix };
})();
